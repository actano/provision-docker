...
(function() {
  ...
  module.exports = function(host, username, options) {
    var proxy, proxyUsername;
    if (options == null) {
      options = {};
    }
    proxy = options.proxy, proxyUsername = options.proxyUsername;
    return {
       ...
       */
      pullImage: Promise.coroutine(function*(tag) {
        return (yield this.dockerClient.pull(tag));
      }),

      /*
          Removes the container with the given `containerName`.
       */
      removeContainer: Promise.coroutine(function*(containerName) {
        yield this.dockerClient.stop(containerName);
        return (yield this.dockerClient.rm(containerName));
      }),
      ...
      replaceContainer: Promise.coroutine(function*(tag, containerName, runConfig) {
        yield this.pullImage(tag);
        yield this.removeContainer(containerName);
