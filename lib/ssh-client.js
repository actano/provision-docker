...
class SSHClient {
  ...
  uploadFile(localPath, remotePath) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var _ref4 = yield _fs2.default.statAsync(localPath);
  ...
  _getSftp() {
    ...
    return _asyncToGenerator(function* () {
      if (_this7._sftp == null) {
        _this7._sftp = yield new _bluebird2.default(function (resolve, reject) {
          _this7.connection.sftp(function (err, sftp) {
            if (err != null) {
              reject(err);
              return;
            }
...
class ProxiedSSHClient extends SSHClient {
  ...
  connect() {
    ...
    return _asyncToGenerator(function* () {
      if (_this8._host == null) {
        throw new Error('no host to proxy to given');
      }
      if (_this8._port == null) {
        throw new Error('no port to proxy to given');
      }

      _this8._proxyClient = new SSHClient(_this8.proxyConfig);
      yield _this8._proxyClient.connect();

      const proxyStream = yield _this8._proxyClient.connection.execAsync(`nc ${_this8._host} ${_this8._port}`);
      _this8.config.sock = proxyStream;
      yield SSHClient.prototype.connect.call(_this8);
    })();
  }
  ...
  close() {
    ...
    return _asyncToGenerator(function* () {
      yield SSHClient.prototype.close.call(_this9);
      yield _this9._proxyClient.close();
    })();
  }
}
